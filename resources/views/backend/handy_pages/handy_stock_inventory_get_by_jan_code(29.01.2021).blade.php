<div class="form-horizontal" id="handy_order_form">
    ...
    <div class="form-group" style="border-radius: 5px;margin-bottom: 10px;">
        @if($result)
    ...
    <div class="" style="width: 100%;">
        <div class="input-group mb-2"
             style="border: .5px solid #b8b7b7;border-radius: 5px;width: 50%;">
            <div class="input-group-prepend"
                 style=" color: black;    /* padding: 0px 0px; */">
                <div class="input-group-text"
                     style="color: black;font-weight: bold;padding: 0 15px;">在庫合計
                </div>
            </div>
            <input type="text" class="total_stock_jaiko_new jaiko_ form-control" readonly=""
                   value="{{$total_jaikos_stock ? $total_jaikos_stock : 0}}" style="padding: 5px 5px;" >
        </div>
    </div>
...
{{--                                    class="total_stock_rack rack_number_{{$value->stock_item_id}}">{{$total_rack_stock}}</td>--}}
                                <td>
                                    <input type="tel" case_invent_qty="" onkeypress="saveAndGoNext(event,{{$key}})" onclick="$(this).select()" onblur="updateInventory({{$key}})"
                                ...
                                <td><input type="tel" bol_invent_qty="" onkeypress="saveAndGoNext(event,{{$key}})" onclick="$(this).select()" onblur="updateInventory({{$key}})"
                                ...
                                <td><input type="tel" unit_invent_qty="" onkeypress="saveAndGoNext(event,{{$key}})" onclick="$(this).select()" onblur="updateInventory({{$key}})"
...
{{--                                    </td>--}}
                                    <td><input type="tel" case_invent_qty="" onkeypress="saveAndGoNext(event,{{$i}})" onclick="$(this).select()" onblur="updateInventory({{$i}},1)"
                                    ...
                                    <td><input type="tel" bol_invent_qty="" onkeypress="saveAndGoNext(event,{{$i}})" onclick="$(this).select()" onblur="updateInventory({{$i}},1)"
                                    ...
                                    <td><input type="tel" unit_invent_qty="" onkeypress="saveAndGoNext(event,{{$i}})" onclick="$(this).select()" onblur="updateInventory({{$i}},1)"
                                    ...
                                    <td style="vertical-align: middle !important;text-align:center;padding:0;">
                                        <input type="tel" class="form-control new_rack_entry{{ $i }}" value="" onkeypress="saveAndGoNext(event)" onclick="$(this).select()" onblur="updateInventory({{$i}},1)"
                                               style="border-radius:0;text-align:center;" onclick="$(this).select()"></td>
                                </tr>
                            @endfor
                        @endif
                    @endforeach
            ...
            <a href="javascript:void(0)" class="btn btn-primary scan_tanarosi_sohin pull-right" onclick="hideModelAndClearInput()" style="float:right">
                次の商品へ</a>
        </div>
    </div>
