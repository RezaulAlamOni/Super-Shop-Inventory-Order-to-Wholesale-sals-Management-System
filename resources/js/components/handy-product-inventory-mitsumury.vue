...
<!--                                                            </th>-->
                                                        ...
                                                        <template v-if="order_data.length > 0">
                                                            <tr v-for="(order,index) in order_data">
                                                                <td>
                                                                    <input type="number" :id="'case'+index"
                                                                           @click="selectItem($event,'ケース')"
                                                                           @keypress="pressEnterAndSave($event,'ball',index)"
                                                                ...
                                                                <td>
                                                                    <input type="number" :id="'ball'+index"
                                                                           @click="selectItem($event,'ケース')"
                                                                           @keypress="pressEnterAndSave($event,'bara',index)"
                                                                ...
                                                                <td>
                                                                    <input type="number" :id="'bara'+index"
                                                                           @click="selectItem($event,'ケース')"
                                                                           @keypress="pressEnterAndSave($event,'case',index)"
...
export default {
    ...
    name: "handy-inventory-mistumury",
    data() {
        return {
            jan_code: '',
            order_data: [],
            ...
            total_quantity: 0,
            handi_navi: '',
            readonly: this.read_only == 1 ? true : false
        }
    },
    mounted() {
        let _this = this;
    ...
    methods: {
        ...
        pressEnterAndSave(e, selector, i) {
            if (e.keyCode == 13) {
                $('#' +selector +i).focus()
                $('#' +selector +i).select()
                if (selector == 'case') {
                    if ($('#' +selector +(i+1)).length <= 0) {
                        $('#order-place-button').focus()
                        $('#order-place-button').select()
                    }
                }
